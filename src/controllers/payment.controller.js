...
export const validationAndMake = async (req, res) => {
  ...
  if (
    ...
    return res.status(400).json({
      message: "Error",
      reason: "Data received was not enough to process transaction",
      data,
    });
  }
  if (monto < 1)
  ...
  if (nroCuotas < 1)
    return res.status(400).json({
      message: "Error",
      reason: "Dues must be greater than zero",
      data,
    });
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "Error", reason: "Internal server error", data });
  }
  if (tran)
    return res.status(400).json({
      message: "OK",
  ...
  try {
    ...
    if (!card)
      return res.status(400).json({
        message: "Error",
        reason: "Card does not exist in database",
        data,
      });
    ...
    if (!owner) {
      return res.status(400).json({
        message: "Error",
        reason: "Person with this document number does not exist in database",
        data,
      });
    }
    if (owner.name !== nombre || owner.email !== email) {
      return res.status(400).json({
        message: "Error",
        reason: "Person name or email doesn't match",
        data,
      });
    }
    if (card.owner !== nombre) {
      return res.status(400).json({
        message: "Error",
        reason: "Person does not own this card",
        data,
      });
    }
    if (
      card.card_type_id !== +mdPago ||
      card.card_franchise_id !== +franquicia
    ) {
      return res
        .status(400)
        .json({ message: "Error", reason: "Bad type or franchise", data });
    }
    if (
      !bcrypt.confirmPassword(card.exp_month, expMonth) ||
      !bcrypt.confirmPassword(card.exp_year, expYear) ||
      !bcrypt.confirmPassword(card.cvv, cv)
    ) {
      return res
        .status(401)
        .json({ message: "Error", reason: "Card authentication failed", data });
    }
